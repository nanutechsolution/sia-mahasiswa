...
<body class="bg-slate-100 font-sans antialiased text-slate-800" x-data="{ sidebarOpen: false }">
    ...
    // Helper Active State Master Data
    $isMasterActive = request()->routeIs(['admin.master.*', 'admin.matakuliah', 'admin.kurikulum*', 'admin.skala-nilai', 'admin.aturan-sks', 'admin.akademik.ekuivalensi-mk', 'admin.perbaikan-nilai']);

    /**
     * Data Menu Admin: Cukup simpan nama ikon Heroicons (Outline)
     */
    $adminSections = [
        [
            ...
            'menus' => [
                ...
                ['label' => 'Komponen Nilai', 'route' => 'admin.komponen-nilai', 'icon' => 'chart-bar'],
        ...
        [
            ...
            'menus' => [
                ...
                ['label' => 'Import Tagihan ', 'route' => 'admin.keuangan.import-tagihan', 'icon' => 'rocket-launch'],
        ...
        [
            ...
            'menus' => [
                ...
                ['label' => 'Indikator', 'route' => 'admin.lpm.indikator', 'icon' => 'presentation-chart-line', 'pattern' => 'admin.lpm.indikator*'],
            ]
        ],
        [
            'title' => 'System & IT',
            ...
            'menus' => [
                ...
                ['label' => 'Audit Logs', 'route' => 'admin.audit', 'icon' => 'list-bullet'],
...
<nav class="flex-1 overflow-y-auto px-4 py-6 space-y-8 sidebar-scroll">
        ...
        @foreach($adminSections as $section)
            @if($user->can($section['permission']) || $role === 'superadmin' || ($role === ($section['role_exception'] ?? '')))
                <div class="space-y-1.5">
                    <p class="px-4 text-[10px] font-bold text-slate-500 uppercase tracking-[0.2em] mb-3">{{ $section['title'] }}</p>
                    
                    @foreach($section['menus'] as $index => $menu)
                        {{-- Sisipkan Master Data Dropdown --}}
                        @if($section['title'] === 'Administrasi Akademik' && $index === 2)
                            <x-nav-dropdown title="Master Data" :active="$isMasterActive" icon="archive-box">
                                @foreach(['admin.master.fakultas' => 'Fakultas', 'admin.master.prodi' => 'Program Studi', 'admin.master.ruang' => 'Data Ruangan', 'admin.matakuliah' => 'Mata Kuliah', 'admin.kurikulum' => 'Kurikulum'] as $route => $label)
                                    <a href="{{ route($route) }}" class="block py-2 text-xs {{ request()->routeIs($route.'*') ? 'text-unmaris-gold font-bold' : 'text-slate-400 hover:text-white' }}" wire:navigate>{{ $label }}</a>
                                @endforeach
                            </x-nav-dropdown>
                        @endif
    ...
    @if($role === 'mahasiswa')
        <div class="space-y-6">
            <div class="space-y-1.5">
                <p class="px-4 text-[10px] font-bold text-slate-500 uppercase tracking-[0.2em] mb-3">Mahasiswa</p>
                ...
                <x-nav-link :href="route('mhs.khs')" :active="request()->routeIs('mhs.khs')" icon="document-text">Kartu Hasil Studi</x-nav-link>
                <x-nav-link :href="route('mhs.keuangan')" :active="request()->routeIs('mhs.keuangan')" icon="credit-card">Keuangan</x-nav-link>
                <x-nav-link :href="route('mhs.profile')" :active="request()->routeIs('mhs.profile')" icon="user-circle">Profil</x-nav-link>
            </div>
        </div>
    @endif
    ...
    @if($role === 'dosen')
        <div class="space-y-6">
            <div class="space-y-1.5">
                <p class="px-4 text-[10px] font-bold text-slate-500 uppercase tracking-[0.2em] mb-3">Dosen</p>
                <x-nav-link :href="route('dosen.jadwal')" :active="request()->routeIs('dosen.jadwal')" icon="calendar">Jadwal</x-nav-link>
                <x-nav-link :href="route('dosen.perwalian')" :active="request()->routeIs('dosen.perwalian*')" icon="users">Perwalian</x-nav-link>
                <x-nav-link :href="route('dosen.profile')" :active="request()->routeIs('dosen.profile')" icon="user-circle">Profil</x-nav-link>
            </div>
        </div>
    @endif
</nav>
