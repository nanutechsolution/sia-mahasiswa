<div class="space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-700">
        ...
        <div class="relative px-8 py-10 sm:px-12 sm:py-12 flex flex-col md:flex-row items-start md:items-center justify-between gap-6">
            <div>
                ...
                <p class="mt-2 text-indigo-100 text-sm md:text-base max-w-xl leading-relaxed">
                    Selamat datang di Sistem Informasi Akademik Terpadu. Akses semua informasi akademik dan administratif Anda dalam satu panel.
                </p>
            </div>
    ...
    {{-- 2. DASHBOARD CONTENT BASED ON ROLE --}}
        ...
        <div class="lg:col-span-3 grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
            ...
            <div class="bg-white p-6 rounded-2xl shadow-sm border border-slate-100 flex items-center gap-4">
                ...
                <div>
                    ...
                    <p class="text-2xl font-black text-slate-800">{{ number_format($stats['ipk'], 2) }}</p>
                </div>
            </div>
            ...
            <div class="bg-white p-6 rounded-2xl shadow-sm border border-slate-100 flex items-center gap-4">
                ...
                <div>
                    ...
                    <p class="text-2xl font-black text-slate-800">{{ $stats['sks_total'] }}</p>
                </div>
            </div>
            ...
            <div class="bg-white p-6 rounded-2xl shadow-sm border border-slate-100 flex items-center gap-4 col-span-1 sm:col-span-2">
                ...
                <div class="flex-1">
                    ...
                    <div class="flex justify-between items-end">
                        ...
                        <p class="text-xs font-medium {{ $stats['sisa_tagihan'] > 0 ? 'text-rose-500' : 'text-green-500' }}">Sisa: Rp {{ number_format($stats['sisa_tagihan'], 0, ',', '.') }}</p>
                    </div>
                </div>
            </div>
        </div>
                ...
                <div class="px-6 py-4 flex items-center group hover:bg-slate-50 transition-colors">
                    ...
                    <div class="flex-1 border-l-2 border-slate-100 pl-4">
                        ...
                        <p class="text-xs text-slate-500 mt-1">
                            R. {{ $detail->jadwalKuliah->ruang }} • {{ $detail->jadwalKuliah->dosen->nama_lengkap_gelar }}
                        </p>
                    </div>
                    <div>
    ...
    <div class="grid grid-cols-1 lg:grid-cols-4 gap-8">
        ...
        <div class="lg:col-span-4 grid grid-cols-1 sm:grid-cols-3 gap-6">
            <div class="bg-white p-6 rounded-2xl shadow-sm border border-slate-200">
                ...
                <p class="text-3xl font-black text-indigo-600 mt-2">{{ $stats['kelas_ajar'] }}</p>
            </div>
            <div class="bg-white p-6 rounded-2xl shadow-sm border border-slate-200">
                <p class="text-xs font-bold text-slate-400 uppercase">Mahasiswa Bimbingan (Aktif)</p>
                <p class="text-3xl font-black text-emerald-600 mt-2">{{ $stats['mhs_wali'] }}</p>
            </div>
            <div class="bg-white p-6 rounded-2xl shadow-sm border border-slate-200 relative overflow-hidden">
                @if($stats['krs_perlu_acc'] > 0)
                ...
                <p class="text-3xl font-black text-rose-600 mt-2">{{ $stats['krs_perlu_acc'] }}</p>
                @if($stats['krs_perlu_acc'] > 0)
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
        <div class="bg-white p-6 rounded-2xl shadow-sm border border-slate-200">
            ...
            <p class="text-2xl font-black text-slate-800 mt-2">{{ $stats['total_mhs_aktif'] }}</p>
        </div>
        <div class="bg-white p-6 rounded-2xl shadow-sm border border-slate-200">
            <p class="text-xs font-bold text-slate-400 uppercase">Pembayaran Pending</p>
            <p class="text-2xl font-black text-amber-500 mt-2">{{ $stats['pembayaran_pending'] }}</p>
        </div>
        <div class="bg-white p-6 rounded-2xl shadow-sm border border-slate-200">
            <p class="text-xs font-bold text-slate-400 uppercase">Kelas Terbuka</p>
            <p class="text-2xl font-black text-indigo-600 mt-2">{{ $stats['kelas_aktif'] }}</p>
        </div>
        <div class="bg-white p-6 rounded-2xl shadow-sm border border-slate-200">
            <p class="text-xs font-bold text-slate-400 uppercase">User Online</p>
            <p class="text-2xl font-black text-emerald-500 mt-2">{{ $stats['user_online'] }}</p>
        </div>
    </div>
