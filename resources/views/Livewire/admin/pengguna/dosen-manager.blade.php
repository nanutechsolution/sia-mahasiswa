<div class="space-y-6">
    <div class="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
            ...
            <p class="text-slate-500 text-sm mt-1">Manajemen biodata, NIDN, dan akun login tenaga pengajar.</p>
        </div>
    ...
    <div class="bg-rose-50 border border-rose-100 p-4 rounded-xl text-rose-800 text-sm font-bold">{{ session('error') }}</div>
    @endif
    ...
    <div class="fixed inset-0 z-50 overflow-y-auto flex items-center justify-center bg-slate-900/60 backdrop-blur-sm p-4 animate-in fade-in duration-200">
        <div class="bg-white rounded-3xl shadow-2xl w-full max-w-lg overflow-hidden border border-white/20">
            <div class="bg-[#002855] px-8 py-6 text-white">
                ...
                <p class="text-[10px] font-bold uppercase opacity-60 mt-2">Support Format Laporan Penugasan Dosen (Warek)</p>
            </div>
            <div class="p-8 space-y-6">
                <div class="bg-slate-50 p-4 rounded-xl border border-slate-100 text-sm text-slate-600">
                    ...
                    <code class="block bg-white p-2 rounded border border-slate-200 text-xs text-indigo-600 font-mono mb-4 break-all">No, NIDN, NUPTK, Nama, Program Studi, L/P, "Tempat,Tanggal Lahir", Agama</code>
                    <button wire:click="downloadTemplate" class="mt-3 text-xs font-bold text-[#002855] hover:underline flex items-center"><svg class="w-4 h-4 mr-1" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4 16v1a3 3 0 003 3h10a3 3 0 003-3v-1m-4-4l-4 4m0 0l-4-4m4 4V4" />
                        </svg> Download Template Contoh</button>
                </div>
                <div>
                    <label class="block text-[10px] font-black text-slate-400 uppercase mb-2">File CSV</label>
                ...
                <div class="pt-4 flex justify-end gap-3">
                    ...
                    <button wire:click="processImport" wire:loading.attr="disabled" class="bg-emerald-600 text-white px-6 py-2 rounded-xl font-bold text-sm shadow-md hover:bg-emerald-700 transition-all flex items-center disabled:opacity-50"><span wire:loading.remove wire:target="processImport">Mulai Import</span><span wire:loading wire:target="processImport">Memproses...</span></button>
                </div>
            </div>
        </div>
    </div>
        ...
        <div class="p-8 space-y-8">
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div class="space-y-4">
                    ...
                    <div>
                        <label class="block text-[11px] font-bold text-slate-500 uppercase tracking-widest mb-2">Nama Lengkap & Gelar *</label>
                        <input type="text" wire:model="nama_lengkap" class="block w-full rounded-lg border-slate-300 focus:border-[#002855] focus:ring-[#002855] text-sm font-bold" placeholder="Nama Dosen">
                    ...
                    <div>
                        ...
                        <input type="email" wire:model="email" class="block w-full rounded-lg border-slate-300 text-sm">
                    </div>
                    <div>
                        <label class="block text-[11px] font-bold text-slate-500 uppercase tracking-widest mb-2">Nomor HP</label>
                        <input type="text" wire:model="no_hp" class="block w-full rounded-lg border-slate-300 text-sm">
                    </div>
                </div>
                ...
                <div class="space-y-4">
                    ...
                    <div class="grid grid-cols-2 gap-4">
                        <div>
                            <label class="block text-[11px] font-bold text-slate-500 uppercase tracking-widest mb-2">Status Kepegawaian</label>
                            <select wire:model.live="jenis_dosen" class="block w-full rounded-lg border-slate-300 focus:border-[#002855] focus:ring-[#002855] text-sm font-bold">
                        ...
                        <div>
                            <label class="block text-[11px] font-bold text-slate-500 uppercase tracking-widest mb-2">Homebase Prodi *</label>
                            <select wire:model="homebase_prodi_id" class="block w-full rounded-lg border-slate-300 focus:border-[#002855] focus:ring-[#002855] text-sm font-bold">
                    ...
                    <div class="bg-indigo-50 p-3 rounded-lg border border-indigo-100 animate-in fade-in">
                        <label class="block text-[11px] font-bold text-indigo-700 uppercase tracking-widest mb-2">Asal Institusi / Perusahaan</label>
                        <input type="text" wire:model="asal_institusi" class="block w-full rounded-lg border-indigo-200 focus:border-[#002855] focus:ring-[#002855] text-sm" placeholder="Contoh: Universitas Indonesia / PT. Telkom">
                    </div>
                    @endif

                    <div class="grid grid-cols-2 gap-4">
                        <div>
                            <label class="block text-[11px] font-bold text-slate-500 uppercase tracking-widest mb-2">NIDN</label>
                            <input type="text" wire:model="nidn" class="block w-full rounded-lg border-slate-300 text-sm font-bold">
                            @error('nidn') <span class="text-rose-500 text-xs font-bold mt-1 block">{{ $message }}</span> @enderror
                        </div>
                        <div>
                            <label class="block text-[11px] font-bold text-slate-500 uppercase tracking-widest mb-2">NUPTK</label>
                            <input type="text" wire:model="nuptk" class="block w-full rounded-lg border-slate-300 text-sm font-bold">
                        </div>
                    </div>
                    <div>
                        ...
                        <input type="password" wire:model="password_baru" class="block w-full rounded-lg border-slate-300 text-sm" placeholder="Min. 6 Karakter">
                        @error('password_baru') <span class="text-rose-500 text-xs font-bold mt-1 block">{{ $message }}</span> @enderror
                    </div>
                    <div class="pt-2">
                        <label class="inline-flex items-center cursor-pointer">
                            <input type="checkbox" wire:model="is_active" class="rounded border-slate-300 text-[#002855] h-5 w-5">
                            <span class="ml-2 text-sm font-bold text-slate-700">Status Aktif</span>
                        </label>
                    </div>
                </div>
            </div>
            ...
            <div class="pt-6 border-t border-slate-100 flex justify-end gap-3">
                <button wire:click="batal" class="px-6 py-2.5 text-sm font-bold text-slate-500 hover:text-slate-600 transition-colors">Batalkan</button>
                <button wire:click="save"
                    class="px-8 py-2.5 bg-[#002855] text-white rounded-xl text-sm font-bold shadow-lg shadow-indigo-200 hover:bg-[#001a38] hover:scale-105 transition-all">
                    Simpan Data
                </button>
            </div>
        </div>
    ...
    <div class="bg-white p-5 shadow-sm rounded-2xl border border-slate-200 grid grid-cols-1 md:grid-cols-2 gap-6">
        ...
        <div>
            ...
            <div class="relative">
                <input type="text" wire:model.live.debounce.300ms="search" placeholder="Nama / NIDN / NUPTK..." class="block w-full rounded-xl border-slate-200 bg-white py-3 pl-10 pr-4 text-sm focus:border-[#002855] focus:ring-[#002855] transition-shadow outline-none font-bold text-slate-700">
                ...
                <tbody class="divide-y divide-slate-50">
                    ...
                    <tr class="hover:bg-slate-50/80 transition-colors group">
                        ...
                        <td class="px-6 py-4 align-top">
                            ...
                            <div class="text-[10px] text-slate-500 italic">Asal: {{ $dosen->asal_institusi }}</div>
                            @endif
                        </td>
                        <td class="px-6 py-4 align-top text-center">
                            <span class="inline-flex items-center px-2.5 py-0.5 rounded-full text-[9px] font-black uppercase tracking-wide {{ $dosen->is_active ? 'bg-emerald-100 text-emerald-700' : 'bg-rose-100 text-rose-700' }}">
                        ...
                        <td class="px-6 py-4 align-top text-right">
                            <div class="flex justify-end gap-2 opacity-60 group-hover:opacity-100 transition-opacity">
                                {{-- Tombol Gelar BARU --}}
                                <button wire:click="openDegreeModal('{{ $dosen->person_id }}')" class="p-2 text-amber-500 hover:bg-amber-50 rounded-lg transition-colors" title="Atur Gelar">
                                ...
                                <button wire:click="edit('{{ $dosen->id }}')" class="p-2 text-[#002855] hover:bg-[#002855]/10 rounded-lg transition-colors" title="Edit">
                                    <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z" />
                                    </svg>
                                </button>
                                <button wire:click="delete('{{ $dosen->id }}')" wire:confirm="Hapus dosen ini? Data login juga akan dihapus." class="p-2 text-rose-600 hover:bg-rose-50 rounded-lg transition-colors" title="Hapus">
                                    <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        ...
        <div class="bg-white rounded-[2.5rem] shadow-2xl w-full max-w-lg overflow-hidden border border-white/20">
            ...
            <div class="p-8 space-y-6">
                ...
                <div class="bg-slate-50 p-6 rounded-3xl border border-slate-100">
                    <div class="grid grid-cols-3 gap-4">
                        <div class="col-span-2">
                            ...
                            <select wire:model="assign_gelar_id" class="w-full rounded-xl border-slate-200 text-sm font-bold text-slate-700">
                        ...
                        <div>
                            ...
                            <input type="number" wire:model="assign_urutan" class="w-full rounded-xl border-slate-200 text-sm text-center font-bold">
                        </div>
                    </div>
                ...
                <div class="space-y-3">
                    ...
                    <div class="flex items-center justify-between p-3 bg-white border border-slate-100 rounded-2xl">
                        <div class="flex items-center gap-3">
                            ...
                            <div>
                                ...
                                <p class="text-[9px] font-bold text-slate-400 uppercase">{{ $ag->posisi }}</p>
                            </div>
                        </div>
                    ...
                    @empty
                    <p class="text-center text-xs text-slate-400 italic py-4">Belum ada gelar terdaftar.</p>
                    @endforelse
                </div>
            </div>

            <div class="p-8 border-t border-slate-50 bg-slate-50/50 text-right">
                <button wire:click="closeModal" class="text-xs font-black text-slate-400 hover:text-slate-600 uppercase tracking-widest">Selesai & Tutup</button>
            </div>
        </div>
    </div>
    @endif
