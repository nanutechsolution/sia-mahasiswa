...
class HRModuleManager extends Component
{
    ...
    public function render()
    {
        ...
        if ($this->showForm && in_array($this->activeTab, ['pegawai', 'penugasan'])) {
            $listPerson = DB::table('ref_person')
                ->where(function ($q) {
                    $q->where('nama_lengkap', 'like', '%' . $this->searchPerson . '%')
    ...
    public function resetForm()
    {
        $this->reset([
            ...
            'activeModal',
            'searchPerson',
            'selectedPersonName' // Reset search state
        ]);
        $this->tanggal_mulai = date('Y-m-d');
        $this->assign_tgl_mulai_role = date('Y-m-d');
    }
    ...
    public function savePerson()
    {
        ...
        if ($this->editMode) DB::table('ref_person')->where('id', $this->personId)->update($data);
            ...
            $data['created_at'] = now();
            DB::table('ref_person')->insert($data);
        }
        $this->resetForm();
        session()->flash('success', 'Data personil disimpan.');
    }
    ...
    public function savePegawai()
    {
        ...
        if ($this->editMode) DB::table('trx_pegawai')->where('id', $this->pegawaiId)->update($data);
            ...
            $data['created_at'] = now();
            DB::table('trx_pegawai')->insert($data);
        }
        $this->resetForm();
        session()->flash('success', 'Data Pegawai disimpan.');
    }
    public function saveRole()
    {
        ...
        if ($this->editMode) DB::table('ref_person_role')->where('id', $this->roleId)->update($data);
            ...
            $data['created_at'] = now();
            DB::table('ref_person_role')->insert($data);
        }
        $this->resetForm();
        session()->flash('success', 'Master Role disimpan.');
    }
    public function editRole($id)
    {
        $r = DB::table('ref_person_role')->find($id);
        $this->roleId = $id;
        $this->kode_role_input = $r->kode_role;
        $this->nama_role_input = $r->nama_role;
        $this->editMode = true;
        $this->showForm = true;
    }
    public function saveJabatan()
    {
        ...
        if ($this->editMode) DB::table('ref_jabatan')->where('id', $this->jabatanId)->update($data);
            ...
            $data['created_at'] = now();
            DB::table('ref_jabatan')->insert($data);
        }
        $this->resetForm();
        session()->flash('success', 'Master Jabatan disimpan.');
    }
    public function editJabatan($id)
    {
        $j = DB::table('ref_jabatan')->find($id);
        $this->jabatanId = $id;
        $this->kode_jabatan = $j->kode_jabatan;
        $this->nama_jabatan_input = $j->nama_jabatan;
        $this->jenis_jabatan = $j->jenis;
        $this->is_active_jabatan = $j->is_active;
        $this->editMode = true;
        $this->showForm = true;
    }
    public function saveGelar()
    {
        ...
        if ($this->editMode) DB::table('ref_gelar')->where('id', $this->gelarId)->update($data);
            ...
            $data['created_at'] = now();
            DB::table('ref_gelar')->insert($data);
        }
        $this->resetForm();
        session()->flash('success', 'Master Gelar disimpan.');
    }
    public function editGelar($id)
    {
        $g = DB::table('ref_gelar')->find($id);
        $this->gelarId = $id;
        $this->kode_gelar = $g->kode;
        $this->nama_gelar_input = $g->nama;
        $this->posisi_gelar = $g->posisi;
        $this->jenjang_gelar = $g->jenjang;
        $this->editMode = true;
        $this->showForm = true;
    }
    public function savePenugasan()
    {
        ...
        if ($this->editMode) DB::table('trx_person_jabatan')->where('id', $this->penugasanId)->update($data);
            ...
            $data['created_at'] = now();
            DB::table('trx_person_jabatan')->insert($data);
        }
        $this->resetForm();
        session()->flash('success', 'Penugasan pejabat disimpan.');
    }
    ...
    public function removeAssignmentRole($id)
    {
        DB::table('trx_person_role')->where('id', $id)->delete();
    }
    ...
    public function downloadTemplate()
    {
        $response = new StreamedResponse(function () {
            ...
            if ($this->activeTab == 'personil') {
                fputcsv($handle, ['Nama Lengkap', 'NIK', 'Email', 'No HP', 'Gender (L/P)']);
                fputcsv($handle, ['Budi Santoso', '3201123456789001', 'budi@mail.com', '08123456789', 'L']);
            } elseif ($this->activeTab == 'pegawai') {
                fputcsv($handle, ['Nama Lengkap (Wajib sama dgn Personil)', 'NIP', 'Jenis Pegawai (TENDIK/ADMIN)', 'Status Aktif (1/0)']);
                fputcsv($handle, ['Siti Aminah', '198001012000121001', 'TENDIK', '1']);
            }
            fclose($handle);
        });
        ...
        try {
            while (($row = fgetcsv($file)) !== false) {
                if ($this->activeTab == 'personil') {
                    ...
                    if (!$person) {
                        $id = DB::table('ref_person')->insertGetId(['nama_lengkap' => trim($row[0]), 'created_at' => now(), 'updated_at' => now()]);
                        $personId = $id;
                    } else {
                        $personId = $person->id;
                    }
